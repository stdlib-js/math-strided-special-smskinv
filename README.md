...
```bash
npm install @stdlib/math-strided-special-smskinv
```
...
```c
#include "stdlib/math/strided/special/smskinv.h"
```

#### stdlib_strided_smskinv( N, \*X, strideX, \*Mask, strideMask, \*Y, strideY )
...
```c
void stdlib_strided_smskinv( const int64_t N, const float *X, const int64_t strideX, const uint8_t *Mask, const int64_t strideMask, float *Y, const int64_t strideY );
```
